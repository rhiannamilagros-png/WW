...
Library.LowGraphicsEnabled = false
Library.AutoRejoinEnabled = false

--------------------------------------------------
-- ACCENT THEMES
--
-- Matches the ScoopHub V2.2 source palettes:
-- Red / Dark / Purple / Ocean.
-- Theme recoloring is event-driven: it only runs when the user
-- changes theme, never on a timer.
--------------------------------------------------
...
local BaseThemeColors = {}

for key, value in pairs(Theme) do
    if typeof(value) == "Color3" then
        BaseThemeColors[key] = value
    end
end
...
local function isPresetFamilyColor(
    color,
    presetName
)
    local preset =
...
local function recoverOriginalRedColor(
    color,
    oldThemeName
)
    ...
    if isPresetFamilyColor(
        color,
        oldThemeName
    ) then
        local _, saturation, value =
            color:ToHSV()

        local _, targetSaturation =
            ThemePresets[
                oldThemeName
            ].Red:ToHSV()

        local _, baseRedSaturation =
            ThemePresets.Red.Red:
                ToHSV()

        local ratio =
            targetSaturation > 0.001
            and (
                targetSaturation
                / math.max(
                    baseRedSaturation,
...
local function transformOriginalThemeColor(
    original,
    themeName
)
    ...
    local _, sourceSaturation, sourceValue =
        original:ToHSV()

    local targetHue, targetSaturation =
        preset.Red:ToHSV()
...
local function transformColorSequence(
    sequence,
    themeName
)
    local points = {}
        ...
        points[#points + 1] =
            ColorSequenceKeypoint.new(
                ...
                isRedFamilyColor(
                    ...
                    and transformOriginalThemeColor(
                        original,
                        themeName
                    )
                    or original
...
local function themePropertyValue(
    instance,
    propertyName,
    rawValue,
    themeName,
    ...
    if typeof(rawValue) == "Color3" then
        local originalAttribute =
            THEME_ORIG_PREFIX
            .. propertyName

        local appliedAttribute =
            THEME_APPLIED_PREFIX
            .. propertyName

        local storedOriginal =
            themeStringToColor(
                instance:
                    GetAttribute(
                        originalAttribute
        ...
        then
            local changedOriginal =
                recoverOriginalRedColor(
                    rawValue,
                    oldThemeName
                        or themeName
                        or "Red"
    ...
    then
        ...
        local appliedAttribute =
            THEME_SEQ_APPLIED_PREFIX
            .. propertyName

        local originalSequence =
            deserializeColorSequence(
                instance:
                    GetAttribute(
                        originalAttribute
                    )
        ...
        local rawSerialized =
            serializeColorSequence(
                rawValue
            )

        local lastSerialized =
            lastAppliedSequence
            and serializeColorSequence(
                lastAppliedSequence
            )
            or nil
...
local function applyDynamicThemeToInstance(
    instance,
    themeName,
    oldThemeName
)
...
local function applyDynamicThemeToTree(
    root,
    themeName,
    oldThemeName
)
...
local function applyThemePalette(
    ...
    -- First recolor every red-family base surface so controls created
    -- after the switch match the selected palette too.
    ...
    -- Exact source-palette accent values.
    Theme.Line =
        preset.Line
...
local function prepareThemedProperties(
    instance,
    props
)
...
function Library:SetTheme(themeName)
    ...
    if oldTheme == themeName then
        return true
    end

    -- Recolor existing windows exactly once for this user action.
    for _, window in ipairs(
        ...
        if window
            ...
            and window.ScreenGui
            and window.ScreenGui.Parent
        then
            applyDynamicThemeToTree(
                window.ScreenGui,
                themeName,
                oldTheme
            )
...
function Library:CreateWindow(config)
    ...
    -- Position the version from the rendered title width instead of a
    -- hard-coded X offset. This prevents "SCOOPHUB2" overlap at scaled sizes.
    task.defer(function()
        if hubTitleLabel.Parent
            and versionLabel.Parent
        then
    ...
    function window:AddTab(tabConfig)
        ...
        function tab:AddSection(sectionConfig, column)
            ...
            local spanColumns =
                tonumber(sectionConfig.Span) == 2
                or selectedColumn == "Full"
                or selectedColumn == "full"

            if spanColumns then
                selectedColumn = 1
            elseif selectedColumn == nil then
                selectedColumn =
        ...
        -- USER DASHBOARD
        --
        -- Recreates the compact ScoopHub V2 User layout:
        --   PLAYER INFO    | SESSION INFO
        --   PREFERENCES    | WEATHER
        --
        -- Weather intentionally has no feature content yet.
        --------------------------------------------------

        function tab:AddUserDashboard(dashboardConfig)
            dashboardConfig =
                dashboardConfig or {}

            self.TitleLabel.Visible =
                false

            self.StatusLabel.Visible =
                false
            ...
            self.ContentTopOffset =
                49

            self.LeftColumn.Position =
                UDim2.new(
                    0,
                    4,
                    0,
            ...
            userHeaderTitle.ZIndex =
                15

            local userHeaderDescription =
                textLabel(
                    self.Scroll,
                    dashboardConfig.Description
                        or "Manage your account, preferences and session.",
                    UDim2.new(
                        0,
                        42,
                        0,
            ...
            track(
                rejoinButton.Activated:
                    Connect(function()
                        if type(
                            dashboardConfig.OnRejoin
                        ) == "function"
                        then
                            dashboardConfig.OnRejoin()
                            return
            ...
            local autoRejoinPreference =
                dashboardToggle(
                    preferenceCard,
                    "Auto Rejoin",
                    "Automatically rejoin after a disconnect.",
                    28,
                    preferences.AutoRejoin
                        == true,
                    function(value)
                        preferences.AutoRejoin =
                            value

                        Library.AutoRejoinEnabled =
                            value

                        if type(
                            dashboardConfig.OnAutoRejoin
                        ) == "function"
                        then
                            dashboardConfig.OnAutoRejoin(
                                value
            ...
            local lowGraphicsPreference =
                dashboardToggle(
                    preferenceCard,
                    "Low Graphics Mode",
                    "Reduce visual effects for better FPS.",
                    56,
                    preferences.LowGraphics
                        == true,
                    function(value)
                        preferences.LowGraphics =
                            value

                        Library.LowGraphicsEnabled =
                            value

                        if type(
                            dashboardConfig.OnLowGraphics
                        ) == "function"
                        then
                            dashboardConfig.OnLowGraphics(
                                value
            ...
            local notificationsPreference =
                dashboardToggle(
                    preferenceCard,
                    "UI Notifications",
                    "Keep ScoopHub notifications enabled.",
                    84,
            ...
            track(
                walkSpeedInput.FocusLost:
                    Connect(function(
                        enterPressed
                    )
                        if type(
                            localConfig.OnWalkSpeedValue
                        ) == "function"
                        then
                            localConfig.OnWalkSpeedValue(
                                walkSpeedInput.Text,
            ...
            local walkSpeedToggle =
                dashboardToggle(
                    ...
                    "",
                    86,
                    localConfig.WalkSpeed
                        == true,
                    function(value)
                        if type(
                            localConfig.OnWalkSpeed
                        ) == "function"
                        then
                            localConfig.OnWalkSpeed(
                                value
            ...
            local infiniteJumpToggle =
                dashboardToggle(
                    ...
                    "",
                    122,
                    localConfig.InfiniteJump
                        == true,
                    function(value)
                        if type(
                            localConfig.OnInfiniteJump
                        ) == "function"
                        then
                            localConfig.OnInfiniteJump(
                                value
            ...
            return {
                ...
                AutoRejoin =
                    autoRejoinPreference,
                LowGraphics =
                    lowGraphicsPreference,
                Notifications =
    ...
    function window:Notify(info, force)
        ...
        if not force
            and Library.NotificationsEnabled
                == false
        then
            return
        end
        ...
        if activeNotice
            and activeNotice.Parent
        then
            activeNotice:Destroy()
        end
        ...
        if noticeAccent ~= "" then
            ...
            task.defer(function()
                if accentTitle.Parent
                    and firstTitle.Parent
                then
                    accentTitle.Position =
                        UDim2.new(
                            0,
                            12
                                + firstTitle
    ...
    track(discordButton.Activated:Connect(function()
        ...
        window:Notify({
            ...
            Content =
                ...
                or "Clipboard is unavailable in this executor.",
            Delay =
                5,
        }, true)

        if type(config.OnDiscordClick) == "function" then
            config.OnDiscordClick(
                discordText,
                copied
            )
        end
