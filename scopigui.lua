 ...
 function window:AddTab(c)
  ...
  function tab:AddUserDashboard(u)
   ...
   local function tog(par,t,d,y,init,cb)
    ...
    if d and d~="" then
     local desc=label(par,d,UDim2.new(0,12,0,y+15),UDim2.new(1,-92,0,15),9,T.Muted,T.Body)
   ...
   local function pv(t,v,y)label(pc,t,UDim2.new(0,114,0,y),UDim2.new(1,-124,0,13),10,T.Muted,T.Body);label(pc,tostring(v),UDim2.new(0,114,0,y+13),UDim2.new(1,-124,0,15),11,T.White,T.Font)end
   ...
   local l=tog(pref,"Low Graphics Mode","Reduce world effects and decorative UI for better FPS.",70,prefs.LowGraphics==true,function(v)prefs.LowGraphics=v;stars.Visible=not v;if type(u.OnLowGraphics)=="function"then u.OnLowGraphics(v)end end)
   local n=tog(pref,"UI Notifications","Keep ScoopHub notification preference enabled.",109,prefs.Notifications==true,function(v)prefs.Notifications=v;window.NotificationsEnabled=v end)
   local tt=tog(pref,"Show Tooltips","Show helper descriptions where supported.",148,prefs.Tooltips==true,function(v)prefs.Tooltips=v;window.TooltipsEnabled=v end)
